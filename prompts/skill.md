...
The prompt registry (`engine_prompts` table) stores agent prompts with version tracking, checksum verification, and usage/error counters. Three endpoints handle CRUD plus utility operations.

### Prompt Types & Roles

| Field | Values | Description |
|-------|--------|-------------|
| `prompt_type` | `MCP`, `Benchmark`, `Magi` | Where the prompt is consumed (MCP server, benchmark runs, future voting engine) |
| `message_role` | `systemprompt`, `usermessage`, `assistant`, `skill` | LLM message role. `systemprompt` = system-level instructions, `usermessage` = user input template, `assistant` = assistant response pattern, `skill` = skill-specific context |

### List Prompts

```bash
# All prompts (sorted by type → prompt_id)
GET /api/v1/prompts

# Filter by type
GET /api/v1/prompts?prompt_type=MCP

# Search by title/prompt_id/description
GET /api/v1/prompts?search=designer
```

Response: `{ "status": "ok", "total": N, "items": [{ id, prompt_id, title, description, content, prompt_type, message_role, file_size, checksum_sha256, version, tags, arguments, usage_counter_since_update, error_counter_since_update, created_at, updated_at }] }`
...
- `prompt_id` (optional): Stable identifier. Auto-generated from title via slugify if omitted (`"My New Prompt"` → `"my-new-prompt"`)
- `prompt_type` (required): One of `MCP`, `Benchmark`, `Magi`
- `message_role` (required): One of `systemprompt`, `usermessage`, `assistant`, `skill`
- `content` (required): Prompt text (max 100KB)
- `description`: Optional description shown in WebUI

Auto-generated `prompt_id` from title: lowercase, non-alphanumeric → hyphens, trim leading/trailing hyphens.

### Update Prompt

```bash
# By stable prompt_id (string path param)
PUT /api/v1/prompts/my-prompt-id
Content-Type: application/json

{ "title": "Updated Title", "content": "New content" }

# By DB row ID (integer path param) — GET only
GET /api/v1/prompts/42
```

- `PUT` uses `<string:prompt_id>` — matches by **stable identifier** (e.g. `designer-system-prompt`)
- `GET /api/v1/prompts/<int:db_id>` matches by **DB row ID**
- Partial updates supported: only provided fields are modified, existing values preserved

### Delete Prompt

```bash
# By DB row ID (integer)
DELETE /api/v1/prompts/42

# By stable prompt_id (string)
DELETE /api/v1/prompts/designer-system-prompt
```

Both paths work: integer DB row ID first, falls back to stable prompt_id string lookup. Also deletes associated `prompt_versions` audit records.

### Utility Endpoints

```bash
# Rescan: re-read core prompts from source code + register custom ones from DB
POST /api/v1/prompts/rescan

# Reset counters (optional prompt_id for single prompt)
POST /api/v1/prompts/reset-counters
POST /api/v1/prompts/reset-counters?prompt_id=designer-system-prompt

# Get content + checksum (for WebUI "Check All" integrity verification)
GET /api/v1/prompts/designer-system-prompt/content
→ { "content": "...", "checksum_sha256": "...", "file_size": 19431 }
```

**Note:** MCP tools do not yet expose prompt CRUD endpoints. Use `quickrobot_api()` directly or curl.
