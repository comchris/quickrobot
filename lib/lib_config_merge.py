...
"""Quickrobot — Canonical config merge module.
...
Functions:
    build_merged_config(db_path, instance_id, node_id=None)
        Build complete merged config with source annotations + model path resolution.
...
from lib.qr_engine_ids import QR_ENGINE_LLAMA_RPC, get_id_by_name
...
# Model path placeholder values that should be treated as empty/omitted
_PLACEHOLDER_PATHS = frozenset(("none", "yes", "no", "true", "false"))
...
def _clean_model_val(val):
    ...
    if val is None:
        return None
    s = str(val).strip()
    if not s or s.lower() in _PLACEHOLDER_PATHS:
        return None
    return s
...
def _resolve_draft_cross_ref(model_dict, db_path):
...
def build_merged_config(db_path, instance_id, node_id=None):
    ...
    The 6-layer chain (lower number = lower precedence):
        Layer 1: Engine default configs (engine_configs table)
        Layer 2: Model definition params (engine_models.model_params — base defaults)
        Layer 3: Preset config template (engine_presets.config_template — overrides model)
    ...
    After all layers merge, model paths are resolved against the active base path.
    ...
    with pool(db_path) as conn:
        ...
        layers = {}  # Source annotation

        # ------------------------------------------------------------------
        # Layer 1: Engine default configs
        # ------------------------------------------------------------------
        engine_configs_rows = conn.execute(
            "SELECT key, value FROM engine_configs WHERE engine_type_id = ?",
            (engine_type_id,),
        ).fetchall()
        layer1_env = {}
        for r in engine_configs_rows:
            layer1_env[r[0]] = r[1]

        merged["env"] = _deep_merge(merged["env"], layer1_env)
        layers["engine_default"] = dict(layer1_env)

        # ------------------------------------------------------------------
        # Layer 2+3: Preset + Model (model_params is base, preset overrides)
        # ------------------------------------------------------------------
        # Load preset data and model params together so we can layer correctly:
        #   1. Engine defaults (already done above)
        #   2. Model definition params — BASE defaults from model record
        #   3. Preset config template — OVERRIDES model defaults
        preset_row = None  # Pre-declare for use in Layer 6 metadata section
        ...
        if preset_id is not None:
                ...
                if model_row:
                    ...
                    if mp:
                        ...
                        if isinstance(model_params, dict):
                            ...
                            merged["model"] = _deep_merge(merged["model"], wrapped)
                            merged["env"] = _deep_merge(merged["env"], wrapped)
                            layers["model"] = {"keys": model_section_keys}
                    ...
                    # draft_model_path may contain "..ID..NNN" cross-reference to another model
                    ...
                    for idx, (env_key, model_key) in path_keys.items():
                        ...
                        if val:
                            merged["env"][env_key] = val
                            merged["model"] = _deep_merge(merged["model"], {model_key: val})
            ...
            if preset_row and preset_row[0]:
                ...
                merged["env"] = _deep_merge(merged["env"], preset_env)
                merged["cli_opts"] = list(merged["cli_opts"]) + preset_cli
                cleaned = _clean_model_dict(preset_model)
                # Resolve ..ID.. cross-references in preset draft model fields
                if db_path and isinstance(cleaned, dict):
                    _resolve_draft_cross_ref(cleaned, db_path)
                merged["model"] = _deep_merge(merged["model"], cleaned)

                # Inject preset gpu_device into merged env as LLAMA_ARG_DEVICE
                if preset_gpu:
                    merged["env"]["LLAMA_ARG_DEVICE"] = preset_gpu

                layers["preset"] = {
                    "env_keys": list(preset_env.keys()) if isinstance(preset_env, dict) else [],
                    "cli_opts_count": len(preset_cli),
                    "model_keys": list(preset_model.keys()) if isinstance(preset_model, dict) else [],
                }
        ...
        if inst["config_override"]:
            ...
            elif is_model_engine and has_flat_override:
            ...
            merged["env"] = _deep_merge(merged["env"], ov_env)
            merged["cli_opts"] = list(merged["cli_opts"]) + ov_cli
            cleaned_ov = _clean_model_dict(ov_model)
            # Resolve ..ID.. cross-references in instance override draft model fields
            if db_path and isinstance(cleaned_ov, dict):
                _resolve_draft_cross_ref(cleaned_ov, db_path)
            merged["model"] = _deep_merge(merged["model"], cleaned_ov)

            layers["instance_override"] = {
                "env_keys": list(ov_env.keys()) if isinstance(ov_env, dict) else [],
                "cli_opts_count": len(ov_cli),
                "model_keys": list(ov_model.keys()) if isinstance(ov_model, dict) else [],
            }

        # ------------------------------------------------------------------
        # Layer 6: Metadata injection
        # ------------------------------------------------------------------
        restart_policy = None
        start_on_boot = False

        # Extract restart_policy from override chain (engine → preset → instance)
        for row in engine_configs_rows:
            if row[0] == "restart_policy" and not restart_policy:
                restart_policy = row[1]

        # Node configs for restart_policy (if node_configs table exists)
        node_configs_rows = conn.execute(
            "SELECT key, value FROM node_configs WHERE node_id = ? AND engine_type_id = ?",
            (_node_id, engine_type_id),
        ).fetchall() if _node_id else []
        for row in node_configs_rows:
            if row[0] == "restart_policy" and not restart_policy:
                restart_policy = row[1]
        ...
        # Start_on_boot resolution: config_override > instance column > engine_configs default
        start_on_boot = False  # default
        sob_raw = None

        # Check config_override.env first (highest priority — user override)
        ...
        # Fallback to instance DB column
        if sob_raw is None:
        ...
        if sob_raw is not None:
            ...
            # Read default from engine_configs table (DB)
            for row in engine_configs_rows:
                if row[0] == "start_on_boot":
                    _cfg_val = str(row[1]).lower()
                    start_on_boot = _cfg_val in ("true", "1", "yes")
                    break

        merged["_restart_policy"] = restart_policy or "no"
        merged["_start_on_boot"] = start_on_boot
        ...
        return {
            ...
            "model": merged["model"],
            "_layers": layers,
            "restart_policy": merged["_restart_policy"],
            "start_on_boot": merged["_start_on_boot"],
        }
