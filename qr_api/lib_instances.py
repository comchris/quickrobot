...
_NODE_BUILD_LOCKS: dict[int, threading.Lock] = {}  # Per-node build locks (replaces old global _NODE_BUILD_LOCK)
_NODE_LOCK_MUTEX = threading.Lock()  # Guards mutation of _NODE_BUILD_LOCKS dict


def get_node_build_lock(node_id: int) -> threading.Lock:
    """Get or create a per-node build lock.

    Ensures only one compile at a time per node (shared cmake build),
    while allowing parallel compiles across different nodes.

    Replaces the old global _NODE_BUILD_LOCK which serialized ALL compiles.

    Args:
        node_id: Node ID to get/create a lock for.

    Returns:
        threading.Lock for this node's build coordination.
    """
    with _NODE_LOCK_MUTEX:
        if node_id not in _NODE_BUILD_LOCKS:
...
def override_system_instance_states(instances, config):
    ...
    for inst in instances:
        ...
        if engine_type_name == QR_ENGINE_API_NAME:
            ...
            # Scheduler: check psutil for actual subprocess lifecycle.
            # If PID is dead and state is error, auto-recover via restart.
            pid = inst.get("pid_last_known") or inst.get("pid")
            if pid and isinstance(pid, int):
                try:
                    p = _psutil.Process(pid)
                    if p.is_running():
                        ...
                        if inst.get("state") == "error":
                            try:
                                ...
                                se = SchedulerEngine()
                                restart_result = se.execute(inst["id"], "restart", config["db_path"])
                                if restart_result.get("status") == "restart_success":
                                    inst["state"] = "starting"
                                    inst["process_age_seconds"] = 0
                                else:
...
def _start_async_build(db_path, instance_id):
    ...
    hostname = (nd.get("ansible_inventory_host") or nd.get("hostname"))
    node_short = hostname.split(".")[0] if hostname else "unknown"

    # Check node build coordination (per-node lock)
    ...
    def _build_task():
        ...
        try:
            ...
            r = _execute_playbook(_resolve_engine_playbook_id("deploy", engine_type_name), resolver_type="playbook_id",
                                  limit=hostname, extra_vars=extra_vars, timeout=QR_TIMEOUT_COMPILE,
...
def _run_manage_action(inst_id, engine_type_name, node_id, action):
    ...
    try:
        r = _execute_playbook("manage_instance", resolver_type="playbook_id",
                               limit=hostname,
                               extra_vars={
                                    "inventory_host": hostname,
                                    "instance_id": inst_id,
                               ...
                               timeout=QR_TIMEOUT_DEFAULT,  # C19: prevents hanging indefinitely
...
def check_remote_uuids(db_path, instance_id):
    ...
    try:
        ...
        if result.returncode != 0:
            results["warnings"].append(f"UUID check failed on node '{target_host}': {result.stderr.strip()}")
            return results
        ...
        for db_name, db_info in db_uuid_map.items():
            ...
            if remote_uuid is None:
                results["warnings"].append(
                    ...
                    f"on node '{nd.get('name', str(node_id))}'"
                )
        ...
        results["warnings"].append(f"UUID check exception on node '{nd.get('name', str(node_id))}': {exc}")
...
def deploy_instance(db_path, instance_id, playbook=None, async_mode=False, skip_build=None):
    ...
    # Determine playbook to use — SSOT engine name keys
    if not playbook:
        _ENGINE_PLAYBOOK_MAP = {
            QR_ENGINE_LLAMA_SERVER_NAME: "DEPLOY_LLAMA_SERVER_V1",
            QR_ENGINE_LLAMA_RPC_NAME: "DEPLOY_LLAMA_RPC_V1",
            QR_ENGINE_IPERF3_NAME: "DEPLOY_IPERF3_V1",
            QR_ENGINE_UNIVERSAL_NAME: "DEPLOY_UNIVERSAL_V1",
        }
        playbook = _ENGINE_PLAYBOOK_MAP.get(engine_type_name)
        if not playbook:
