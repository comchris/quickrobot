...
# quickrobot/__init__.py is at /path/to/quickrobot/quickrobot/__init__.py
...
def _check_pid_file():
    ...
    try:
        ...
        return pid, f"quickrobot already running on port {port} (PID {pid}). Use --replace."
    except OSError:
        return None, None
...
from quickrobot.lib_instances import _execute_playbook, deploy_instance
...
def register_routes(app):
    ...
    app.add_url_rule("/api/v1/instances/<int:inst_id>/update-build", "api_instance_update_build", api_instance_update_build, methods=["POST"])
